...
echo "$($PSQL "SELECT SUM(winner_goals)::numeric / count(winner_goals) FROM games")"

echo -e "\nAverage number of goals in all games from the winning teams rounded to two decimal places:"
echo "$($PSQL "SELECT round(SUM(winner_goals)::numeric / count(winner_goals), 2) FROM games")"

echo -e "\nAverage number of goals in all games from both teams:"
echo "$($PSQL "SELECT (SUM(winner_goals)::numeric + SUM(opponent_goals)) / count(winner_goals) FROM games")"
